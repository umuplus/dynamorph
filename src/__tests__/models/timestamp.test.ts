...
test('basic timestamp attribute', () => {
    const timestamp = new TimestampType('prop', { on: TimestampOn.Values.CREATE, type: Timestamp.Values.ISO_STRING })
    const val = new Date().toISOString()
    timestamp.setValue(val)
    expect(timestamp.propertyName).toEqual('prop')
...
test('timestamp attribute in milliseconds', () => {
    const timestamp = new TimestampType('prop', { on: TimestampOn.Values.CREATE, type: Timestamp.Values.MILLISECONDS })
...
test('timestamp attribute in seconds', () => {
    const timestamp = new TimestampType('prop', { on: TimestampOn.Values.CREATE, type: Timestamp.Values.SECONDS })
